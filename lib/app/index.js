...
App.prototype.addArticle = function(userId, url, action, clotheType) {
  var id = uuid.v1();
  this.connections.queue.publish(SCRAPE_QUEUE, { id: id, url: url, userId: userId ,action : action, clotheType : clotheType});
  return Promise.resolve(id);
};

App.prototype.scrapeArticle = function(userId, id, url, action, clotheType) {
  logger.log({ type: 'info', msg:+"1" + action + " clotheType:" +  clotheType});
  return this.scraper(userId, id, url, action, clotheType);
};
...
App.prototype.handleScrapeJob = function(job, ack) {
  ...
  this
    .scrapeArticle(job.userId, job.id, job.url, job.action, job.clotheType)
