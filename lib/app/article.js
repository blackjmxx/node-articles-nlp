...
module.exports = function scrape(userId, id, url, action, clotheType) {
    return new Promise(function (resolve, reject) {
        if (url != undefined) {
            if (action == "ZALANDO") {
                try {
                browser.requestSingle({
                    ...
                    superagent
                        //.post('https://linkshopperparse.herokuapp.com/parse/classes/tempurl')
                        .post('https://pg-app-74w9kvxmpn4wk9x0hy3cxurvreesex.scalabl.cloud/1/classes/tempurl')
                        .send(data)
                        .set('X-Parse-Application-Id', 'DMQiCsR4mUSZtmGpxEZvykmCQhG3R9PKPqSjRWWQ')
                        .set('X-Parse-REST-API-Key', 'ydNpcrzNQ8O36eDhWXG7vAHb9QGRHRPlPPS8HhWA')
                        .set('Content-Type', 'application/json')
                        .end(function (err, res) {
                            if (err || !res.ok) {
...
// soldPrice:$('body > div:nth-child(5) > div:nth-child(11) > div > div > div > z-grid:nth-child(1) > z-grid-item.z-vegas_primaryContent_productInfo > div.zvui-product-card.zvui-main-info > div > div.zvui-product-price.price-right.large > div:nth-child(3) > span.z-text.zvui-product-price-display.zvui-product-price-actual.large.sale.z-text-title-3.z-text-black')')
